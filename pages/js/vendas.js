document.addEventListener('DOMContentLoaded', async () => {
  ...
  function adicionarVenda(venda) {
    ...
    row.insertCell(0).innerText = venda.produto;
    row.insertCell(1).innerText = venda.preco_custo;
  ...
  function exportarParaPDF() {
    ...
    const doc = new jsPDF();
    doc.text("Lista de Vendas", 20, 20);
    let rowIndex = 30;
    for (let i = 0; i < rows.length; i++) {
      const cells = rows[i].getElementsByTagName('td');
      let row = [];
  ...
  aplicarFiltros.addEventListener('click', aplicarFiltrosVendas);
